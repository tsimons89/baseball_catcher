...
int RoboteqDevice::Connect(string port)
{
	...
	char P[16];
	sprintf_s(P, 16, "\\\\.\\%s", port.c_str());		// CreatFile only allows COM1~COM9 for COM10 it must be \\\\.\\COM10
	HANDLE h = CreateFileA(P, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	handle = (int) h;
